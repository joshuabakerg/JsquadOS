...
--    print("___________________________________________________")
	local fileToLoad = josCfg.desktopDirectory  --sets the desktop directory to the config file
	local file = null
	while true do 
		...
		local file ,button ,x ,y= jos.getPressedButton(fileToLoad)
		if file ~= null then
			if button ~= 2 then
				if fs.isDir(fileToLoad..file) then
					shell.run("cd "..fileToLoad..file)
					josMulti.changeCurrentTabI(josMulti.newTab("/usr/bin/guiExplorer.lua "..file,fs.getName("/usr/bin/guiExplorer.lua")))
					josMulti.runTab()
				end		
			else
				jos.drawOptionPopUpMenu(x,y,file)
			end
		end
